...
class Select extends React.Component {
  render() {
    const { data, onChange, placeholder, value, label, bold, info, success, warning, error, noMargin, style, disabled, offlineAware, isOffline, ...props } = this.props;
    ...
    if (disabled || (isOffline() && offlineAware)) return (
      <View style={[{ marginTop: noMargin ? 0 : 10 }, style]}>
        ...
        <View style={[styles.container, additionalContainerStyle]}>
          <Text placeholder={!value} style={[styles.textfield, additionalTextfieldStyle]}>{showValue.label || placeholder}</Text>
          <Icon name="icon-dropdown" info style={{ position: 'absolute', right: 5, top: (Platform.OS === 'ios') ? 4 : 12 }} disabled />
        </View>
      </View>
    );
    ...
    return (
      <ModalPicker
        ...
        <View style={[{ marginTop: noMargin ? 0 : 10 }, style]}>
          ...
          <View style={[styles.container, additionalContainerStyle]}>
            <Text placeholder={!value} style={[styles.textfield, additionalTextfieldStyle]}>{showValue.label || placeholder}</Text>
            <Icon name="icon-dropdown" info style={{ position: 'absolute', right: 5, top: (Platform.OS === 'ios') ? 4 : 12 }} />
          </View>
        </View>
      </ModalPicker>
    );
...
const Extended = visualProvided(Select);
